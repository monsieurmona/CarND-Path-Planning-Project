...
//   const double t0 = - (updateInterval);
...
//*/
//   const double maxAcceleration = std::pow(speedOffsetInMps,2) / (2 * sSection);
//   const double accelerationDiff = lastAcceleration - maxAcceleration;
//   const double maxSpeedChangeDuration = abs(speedOffsetInMps / abs(maxAcceleration));
//   const double a = abs(maxAcceleration) * ((speedOffsetInMps < 0.0) ? -1.0 : 1.0);

//   const double t0 = - (updateInterval);
//   const double t1 = t0 + updateInterval;
//   const double t2 = t1 + maxSpeedChangeDuration / 3.0;
//   const double t3 = t2 + 2 * (maxSpeedChangeDuration / 3.0);
//   const double t4 = t3 + updateInterval;

//   m_speedPoints.push_back(Coordinate2D(t0, speedInMps0      ));
//   m_speedPoints.push_back(Coordinate2D(t1, speedInMps1      ));
//   m_speedPoints.push_back(Coordinate2D(t2, speedInMps1 + speedOffsetInMps / 2.0 ));
